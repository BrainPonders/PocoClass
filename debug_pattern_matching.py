...
def test_individual_patterns():
    ...
    # Load the Rabobank rule
    rules = rule_loader.load_all_rules()
    rabobank_rule = rules.get('rabobank_check_592')
    
    if not rabobank_rule:
        print("Failed to load Rabobank rule")
        return
    ...
    core_identifiers = rabobank_rule.get('core_identifiers', {})
            ...
            try:
                if re.search(pattern, test_text, re.IGNORECASE):
                    ...
                    if source == 'content' and pattern:
                        ...
                        if 'rabobank' in pattern_lower and 'rabobank' in text_lower:
                            pos = text_lower.find('rabobank')
                            print(f"    Found 'rabobank' at position {pos}")
    ...
    evaluation = pattern_matcher.evaluate_rule(rabobank_rule, content, filename)
