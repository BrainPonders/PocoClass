...
const DocumentBrowser = ({ onNewRule, onEditRule, onTestRules }) => {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      ...
      if (correspondentRef.current && !correspondentRef.current.contains(event.target)) {
  ...
  const mockDocuments = [
    ...
    {
      id: '2000-10-30-ExampleBank_Credit Card',
      title: 'examplebank_credit_statement.pdf',
  ...
  const availableCorrespondents = ['My Bank', 'Supplier ABC', 'Office Store', 'Electric Company', 'ExampleBank']
        ...
        <div className="flex items-center" style={{marginTop: '40px', marginBottom: '40px'}}>
          <div className="flex" style={{marginLeft: '20px'}}>
            ...
            <div className="relative" style={{marginRight: '20px'}} ref={tagsRef}>
              ...
              {showTagsFilter && (
                <div className="absolute top-full left-0 mt-1 rounded shadow-lg border z-10" style={{backgroundColor: 'var(--paperless-surface)', border: '0.5px solid var(--paperless-border)', width: '350px', minHeight: '400px'}}>
                  {/* Include/Exclude Toggle */}
                  <div className="flex" style={{borderBottom: '0.5px solid var(--paperless-border)'}}>
                    <button className="flex-1 py-3 px-4 text-base font-medium" style={{backgroundColor: 'var(--paperless-accent)', color: '#000'}}>Include</button>
                    <button className="flex-1 py-3 px-4 text-base font-medium" style={{backgroundColor: 'var(--paperless-surface)', color: 'var(--paperless-text-secondary)'}}>Exclude</button>
                  </div>
                  {/* Search Filter */}
                  <div className="p-4" style={{borderBottom: '0.5px solid var(--paperless-border)'}}>
                    <input 
                      type="text" 
                      placeholder="Filter tags" 
                      className="w-full px-4 py-3 text-base rounded" 
                      style={{backgroundColor: 'var(--paperless-surface-light)', border: '0.5px solid var(--paperless-border)', color: 'var(--paperless-text)'}} 
                    />
                  </div>
                  {/* Tags List */}
                  <div className="overflow-y-auto" style={{maxHeight: '300px'}}>
                    <div className="px-4 py-5 text-base font-medium" style={{color: 'var(--paperless-text-secondary)', borderBottom: '0.5px solid var(--paperless-border)'}}>
                      Not assigned
                    </div>
                    {availableTags.map(tag => (
                      <button 
                        key={tag} 
                        className="flex items-center justify-between w-full px-4 py-5 text-base cursor-pointer hover:bg-opacity-20 text-left transition-colors"
                        style={{
                          backgroundColor: selectedTags.includes(tag) ? 'var(--paperless-surface-light)' : 'transparent',
                          color: 'var(--paperless-text)',
                          borderBottom: '0.5px solid var(--paperless-border)'
                        }}
                        onMouseEnter={(e) => {
                      ...
                      >
                        ...
                        <span className="text-base font-medium" style={{color: 'var(--paperless-text-secondary)'}}>0</span>
                      </button>
                    ))}
                  </div>
                </div>
                ...
                <div className="absolute top-full left-0 mt-1 rounded shadow-lg border z-10" style={{backgroundColor: 'var(--paperless-surface)', border: '0.5px solid var(--paperless-border)', width: '350px', minHeight: '400px'}}>
                  ...
                  <div className="overflow-y-auto" style={{maxHeight: '300px'}}>
                    ...
                    {availableCorrespondents.map(correspondent => (
                      <button 
                        key={correspondent} 
                        className="flex items-center justify-between w-full px-4 py-5 text-base hover:bg-opacity-20 text-left transition-colors"
                        style={{
                          color: 'var(--paperless-text)', 
                      ...
                      >
                        ...
                        <span className="text-base font-medium" style={{color: 'var(--paperless-text-secondary)'}}>190</span>
                      </button>
                    ))}
                  </div>
                </div>
                  ...
                  <div className="overflow-y-auto" style={{maxHeight: '300px'}}>
                    ...
                    {availableDocTypes.map(docType => (
                      ...
                      >
                        ...
                        <span className="text-base font-medium" style={{color: 'var(--paperless-text-secondary)'}}>2</span>
                      </button>
                    ))}
                  </div>
                </div>
