...
if [ ! -f "$REPO_DIR/maintainer/docker/Dockerfile" ]; then
    echo "ERROR: Dockerfile not found at $REPO_DIR/maintainer/docker/Dockerfile"
    exit 1
fi
...
docker build \
    ...
    -t "${BUILD_IMAGE_TAG}" \
    -t "${SHA_IMAGE_TAG}" \
    -f "$REPO_DIR/maintainer/docker/Dockerfile" \
    "$REPO_DIR"
